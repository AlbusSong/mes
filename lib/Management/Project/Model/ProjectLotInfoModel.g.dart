// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'ProjectLotInfoModel.dart';

// **************************************************************************
// JsonSerializableGenerator
// **************************************************************************

ProjectLotInfoModel _$ProjectLotInfoModelFromJson(Map<String, dynamic> json) {
  return ProjectLotInfoModel(
    json['LOTSize'] as int,
    json['StatusDesc'] as String,
    json['HoldDesc'] as String,
    json['CTime'] as String,
    json['OTime'] as String,
    json['LevelManage'] as String,
    json['ProdClassCode'] as String,
    json['LotNo'] as String,
    json['Status'] as int,
    json['Hold'] as String,
    json['HoldCode'] as String,
    json['ProcessCode'] as String,
    json['ProcessName'] as String,
    json['ItemCode'] as String,
    json['ItemName'] as String,
    json['Dscb'] as String,
    json['Qty'] as int,
    json['Cost'] as int,
    json['CToolNo'] as String,
    json['Wono'] as String,
    json['OrderNo'] as String,
    json['Grade'] as String,
    json['LineCode'] as String,
    json['LineName'] as String,
    json['WorkCenterCode'] as String,
    json['WorkCenterName'] as String,
    json['Barcode'] as String,
    json['Delflag'] as bool,
    json['CreateTime'] as String,
    json['ModifiedTime'] as String,
    json['Comment'] as String,
    json['DataGroup'] as String,
  );
}

Map<String, dynamic> _$ProjectLotInfoModelToJson(
        ProjectLotInfoModel instance) =>
    <String, dynamic>{
      'LOTSize': instance.LOTSize,
      'StatusDesc': instance.StatusDesc,
      'HoldDesc': instance.HoldDesc,
      'CTime': instance.CTime,
      'OTime': instance.OTime,
      'LevelManage': instance.LevelManage,
      'ProdClassCode': instance.ProdClassCode,
      'LotNo': instance.LotNo,
      'Status': instance.Status,
      'Hold': instance.Hold,
      'HoldCode': instance.HoldCode,
      'ProcessCode': instance.ProcessCode,
      'ProcessName': instance.ProcessName,
      'ItemCode': instance.ItemCode,
      'ItemName': instance.ItemName,
      'Dscb': instance.Dscb,
      'Qty': instance.Qty,
      'Cost': instance.Cost,
      'CToolNo': instance.CToolNo,
      'Wono': instance.Wono,
      'OrderNo': instance.OrderNo,
      'Grade': instance.Grade,
      'LineCode': instance.LineCode,
      'LineName': instance.LineName,
      'WorkCenterCode': instance.WorkCenterCode,
      'WorkCenterName': instance.WorkCenterName,
      'Barcode': instance.Barcode,
      'Delflag': instance.Delflag,
      'CreateTime': instance.CreateTime,
      'ModifiedTime': instance.ModifiedTime,
      'Comment': instance.Comment,
      'DataGroup': instance.DataGroup,
    };
